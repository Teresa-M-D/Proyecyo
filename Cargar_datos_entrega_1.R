...
paquetes <- c(
  ...
  "plotly", "scatterplot3d", "vcd", "lsr", "GGally", "psych", "scales", "sf", "rnaturalearth", "rnaturalearthdata", "car", "pROC", "caret", "tidyr", "rstatix"
)
...
datos_recodificados$Nationality<-dplyr::recode(
  as.character(datos_recodificados$Nationality),
  `1` = "Portugués",
  `2` = "Alemán",
  `6` = "Español",
  `11` = "Italiano",
...
#Course
datos_recodificados$Course <- dplyr::recode(
  as.character(datos_recodificados$Course),
...
datos_recodificados$Mother.s.qualification <- dplyr::recode(
  as.character(datos_recodificados$Mother.s.qualification),
...
datos_recodificados$Father.s.qualification <- dplyr::recode(
  as.character(datos_recodificados$Father.s.qualification),
  `1` = "Educación secundaria - 12º año o equivalente",
  `2` = "Educación superior - Grado (Bachelor)",
  `3` = "Educación superior - Grado",
  `4` = "Educación superior - Máster",
...
datos_recodificados$Mother.s.occupation <- dplyr::recode(
  as.character(datos_recodificados$Mother.s.occupation),
  `0` = "Estudiante",
...
#Father's occupation:
datos_recodificados$Father.s.occupation <- dplyr::recode(
  as.character(datos_recodificados$Father.s.occupation),
  `0` = "Estudiante",
  `1` = "Representantes del poder legislativo y ejecutivo, directores y gerentes",
  `2` = "Especialistas en actividades intelectuales y científicas",
  `3` = "Técnicos y profesiones de nivel intermedio",
...
datos_recodificados$Displaced <- dplyr::recode(
  as.character(datos_recodificados$Displaced),
  `1` = "Sí",
  `0` = "No")


#Education special needs:
datos_recodificados$Educational.special.needs <- dplyr::recode(
  as.character(datos_recodificados$Educational.special.needs),
  `1` = "Sí",
  `0` = "No")

#Debtor:
datos_recodificados$Debtor<-dplyr::recode(
  as.character(datos_recodificados$Debtor),
  `1` = "Sí",
  `0` = "No")

#Tution feets up:
datos_recodificados$Tuition.fees.up.to.date <- dplyr::recode(
  as.character(datos_recodificados$Tuition.fees.up.to.date),
  `1` = "Sí",
  `0` = "No")

#International:
datos_recodificados$International<-dplyr::recode(
  as.character(datos_recodificados$International),
  `1` = "Sí",
  `0` = "No")

#Gender:
datos_recodificados$Gender <- dplyr::recode(
  as.character(datos_recodificados$Gender),
  `1` = "Masculino",
  `0` = "Femenino")

#Scholarship holder:
datos_recodificados$Scholarship.holder <- dplyr::recode(
  as.character(datos_recodificados$Scholarship.holder),
  `1` = "Sí",
  `0` = "No")

#Marital Status:
datos_recodificados$Marital.status <- dplyr::recode(
  as.character(datos_recodificados$Marital.status),
  `1` = "Soltero",
  `2` = "Casado",
  `3` = "Viudo",
  `4` = "Divorciado",
...
#Application mode:
datos_recodificados$Application.mode<-dplyr::recode(
  as.character(datos_recodificados$Application.mode),
...
datos_recodificados$Previous.qualification <- dplyr::recode(
  as.character(datos_recodificados$Previous.qualification),
...
datos_recodificados$Daytime.evening.attendance. <- dplyr::recode(
  as.character(datos_recodificados$Daytime.evening.attendance.),
  `1` = "Mañana",
  `0` = "Tarde")
